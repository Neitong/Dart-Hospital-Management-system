...
class AppointmentMenu {
  ...
  AppointmentMenu(this._db) : _appointmentService = AppointmentService(_db);
  ...
  void showAppointmentsMenu() {
    ...
    while (running) {
      ...
      print(
        '${ConsoleUtils.cyan}[1]${ConsoleUtils.reset} Schedule New Appointment',
      );
      print(
        '${ConsoleUtils.cyan}[2]${ConsoleUtils.reset} View All Appointments',
      );
  ...
  void _scheduleAppointment() {
    ...
    DateTime date;
    //Check the date is not valid. If user input the date past throw the error immediately
    try {
    ...
    final enteredDate = DateTime(date.year, date.month, date.day);

    //check the date is not in the past
    if (enteredDate.isBefore(today)) {
      ConsoleUtils.printError(
          'Cannot schedule appointments in the past. Please enter today or a future date.');
      ConsoleUtils.waitForKey();
      return; // Go back as requested
    }

    //Check available slots for today
    final availableSlots = _appointmentService.getAvailableSlots(
      selectedDoctor.id,
      DateTime.parse(dateStr),
    );

    //Check available slots is not empty.
    if (availableSlots.isEmpty) {
      ConsoleUtils.printError(
        'Dr. ${selectedDoctor.name} has no available 1-hour slots on this day.',
      );
      ConsoleUtils.waitForKey();
      return;
    }
    ...
    ConsoleUtils.printInfo(
      'Available 1-hour slots for Dr. ${selectedDoctor.name} on $dateStr (8:00 AM - 5:00 PM):',
    );
    for (int i = 0; i < availableSlots.length; i++) {
      final slot = availableSlots[i];
      final time = '${slot.hour.toString().padLeft(2, '0')}:00';
      print('  ${ConsoleUtils.cyan}[${i + 1}].${ConsoleUtils.reset} $time');
    }

    final slotIndex = ConsoleUtils.readInt('Enter Slot Number: ');
    if (slotIndex == null ||
        slotIndex < 1 ||
        slotIndex > availableSlots.length) {
      ConsoleUtils.printError('Invalid slot selection.');
      ConsoleUtils.waitForKey();
      return;
    }

    final selectedTime = availableSlots[slotIndex - 1];
  ...
  void _viewAllAppointments() {
    ...
    if (appointments.isEmpty) {
      ...
      print(
        '${ConsoleUtils.cyan}${'-' * (idWidth + patientWidth + doctorWidth + dateWidth + timeWidth + statusWidth)}${ConsoleUtils.reset}',
      );
  ...
  void _cancelAppointment() {
    ...
    final scheduledAppointments = _db
        .getAllAppointments()
    ...
    final success = _appointmentService.cancelAppointment(
